...
path = os.path.expanduser("~/Datasets/imagenet/train")
targets = []
for category in os.listdir(path):
    images = random.sample(os.listdir(os.path.join(path, category)), 3)
    targets += images

random.shuffle(targets)

backgrounds = ["random", "rev_comp", "rev_msemax", "white"]
tot = 0

for goal in targets:
    for background in backgrounds:
        args.canvas_setup = background
        dir_name = goal.split("_")[0]
        goal_path = os.path.join(path, dir_name, goal)
        ...
        result_dir = f'{goal_name}'
        result_path = f'results/{result_dir}'
        ...
        for ss, (target_width, target_height) in enumerate(zip(scale_order, scale_order)):
            ...
            # Save scale progress
            result_fname = f'{background}.jpg'
            result_fname = os.path.join(result_path, result_fname)
            full_canvas = np.clip(full_canvas, 0, 1)
            cv2.imwrite(result_fname, cv2.cvtColor(np.uint8(full_canvas*255), cv2.COLOR_BGR2RGB))
    
    tot += 1
    slack.notify(text=f"Done {tot} / {len(targets)}")